/**
 * Geometrie Quest-Aufgaben (Standard) basierend auf MUNDO-Inhalten
 *
...
/**
 * TRIGONOMETRIE - Quest-Aufgaben für neue Unit u7 oder Erweiterung von u2
 */
export function createTrigonometrieQuest(index: number, seed: number): Task {
  const scenarios = [
    {
      ...
      a: (a: number, c: number) => ((a / c) * 100).toFixed(0) / 100,
      e: 'Sinus = Gegenkathete / Hypotenuse',
    },
    {
      type: 'input' as const,
      q: (b: number, c: number) =>
        `Rechtwinkliges Dreieck: Ankathete = ${b} cm, Hypotenuse = ${c} cm. Berechne cos(β) (auf 2 Dezimalstellen gerundet).`,
      a: (b: number, c: number) => ((b / c) * 100).toFixed(0) / 100,
    ...
    },
    {
      ...
      a: (a: number, b: number) => ((a / b) * 100).toFixed(0) / 100,
  ...
  if (scenario.type === 'choice') {
    ...
    if (index % scenarios.length === 0) {
      ...
      answer = scenario.a(a, c).toString();
    } else if (index % scenarios.length === 1) {
      question = scenario.q(b, c);
      answer = scenario.a(b, c).toString();
    } else {
      question = scenario.q(a, b);
      answer = scenario.a(a, b).toString();
    }
...
export function createPythagorasQuest(index: number, seed: number): Task {
  const scenarios = [
    ...
    },
    {
      ...
      a: (p: number, q: number) => Math.sqrt(p * q).toFixed(1),
      e: 'Höhensatz: h² = p × q',
    },
    {
      type: 'input' as const,
      ...
      a: (c: number, p: number) => Math.sqrt(p * c).toFixed(1),
      e: 'Kathetensatz: a² = p × c',
    },
  ];

  const scenario = scenarios[index % scenarios.length];
  const id = `u6-pythagoras-${index}-${seed}`;
  ...
  return {
    ...
    validator: {
      type: typeof answer === 'number' ? 'numeric' : 'numericTolerance',
      numericAnswer: typeof answer === 'number' ? answer : parseFloat(answer),
      tolerance: typeof answer === 'number' ? 0 : 0.1,
    },
  };
}
...
export function createKoerperQuest(index: number, seed: number): Task {
  ...
  const scenario = scenarios[index % scenarios.length];
  const id = `u4-koerper-${index}-${seed}`;
...
/**
 * KONGRUENTE DREIECKE - Quest-Aufgaben für u2 oder u5
 */
export function createKongruenzQuest(index: number, seed: number): Task {
  ...
  const scenario = scenarios[index % scenarios.length];
  const id = `u2-kongruenz-${index}-${seed}`;
...
export function createVieleckQuest(index: number, seed: number): Task {
  ...
  const scenario = scenarios[index % scenarios.length];
  const id = `u3-vieleck-${index}-${seed}`;
...
/**
 * 3D-GEOMETRIE - Quest-Aufgaben für neue Unit u8
 */
  ...
  const scenarios = [
    {
      ...
      a: (x1: number, y1: number, z1: number, x2: number, y2: number, z2: number) =>
        Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2 + (z2 - z1) ** 2).toFixed(1),
 ...
 *   if (unitId === 'u7') {
 *     pool.push(...Array.from({length: 5}, (_, i) => createTrigonometrieQuest(i, Date.now())));
 *   }
 *   if (unitId === 'u8') {
 *     pool.push(...Array.from({length: 5}, (_, i) => createDreidQuest(i, Date.now())));
 *   }
 *   // Erweiterungen:
 *   if (unitId === 'u2') {
 ...
 *   }
 *   if (unitId === 'u4') {
 *     pool.push(...Array.from({length: 3}, (_, i) => createKoerperQuest(i, Date.now())));
 *   }
 *   if (unitId === 'u5') {
 *     pool.push(...Array.from({length: 2}, (_, i) => createStrahlensatzQuest(i, Date.now())));
 *   }
 *   if (unitId === 'u6') {
 *     pool.push(...Array.from({length: 2}, (_, i) => createPythagorasQuest(i, Date.now())));
 *   }
 *   if (unitId === 'u3') {
