...
// Führe aus mit: node test-supabase.mjs
// Oder: npm run test:supabase

import { createClient } from "@supabase/supabase-js";

// Optional: Lade .env.local oder .env wenn dotenv installiert ist
...
async function testConnection() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    ...
    if (!dotenvLoaded) {
      console.log(
        "\n💡 Tipp: Installiere dotenv für .env Support: npm install --save-dev dotenv"
      );
    }
    process.exit(1);
  }
  ...
  console.log(
    `📍 URL: ${
      supabaseUrl ? supabaseUrl.substring(0, 30) + "..." : "NICHT GESETZT"
    }`
  );
  console.log(
    `🔑 Key: ${
      supabaseKey ? supabaseKey.substring(0, 20) + "..." : "NICHT GESETZT"
    }\n`
  );
