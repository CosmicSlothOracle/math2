...
describe("Answer Validators", () => {
  ...
  it("matches keywords with negation", () => {
    expect(
      matchKeywords("Keine rechten Winkel vorhanden", ["rechten winkel"])
    ).toBe(true);
    expect(matchKeywords("hat rechte Winkel", ["rechte winkel"], "any")).toBe(
      true
    );
  });
  ...
  it("validates numeric tolerance answers", () => {
    const config = {
      type: "numericTolerance" as const,
      numericAnswer: 6280,
      tolerance: 10,
    };
  ...
  it("validates equations via pattern match", () => {
    expect(matchKeywords("Rectangle", ["rechteck"])).toBe(true);
  });

  it("handles default numeric tolerance (0.1)", () => {
    const config = { type: "numericTolerance" as const, numericAnswer: 10 };
    expect(validateAnswer("10.05", config)).toBe(true);
    expect(validateAnswer("10.2", config)).toBe(false);
  });
...
describe("Freeform Answer Evaluation", () => {
  ...
  it("flattens JSON answers", () => {
    ...
    const result = evaluateFreeformAnswer(
      "Ein Quadrat mit rechten Winkeln",
      correct
    );
    expect(result.isMatch).toBe(true);
  });
